...
const NCMisMtrReportDao = () => {


  const getNCMisMtrReportDao = async (req) => {
    return new Promise(async function (resolve) {
      ...
      WHERE "isActive" = TRUE
      AND (date(concat(l1."year"::varchar,'-',l1."month"::varchar,'-01')) BETWEEN '${startDate}' AND '${endDate}')
      GROUP BY "nameOfUnit",
             "districtId") dis
      ...
      AND (date(concat(m2."year"::varchar,'-',m2."month"::varchar,'-01')) BETWEEN '${startDate}' AND '${endDate}')
      GROUP BY "nameOfUnit",
             "districtId") mfTreat
  ...
  return {
    getNCMisMtrReportDao
  };
};
export default NCMisMtrReportDao();
